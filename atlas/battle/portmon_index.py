        ...
        for b in raw:
            ...
            if s and pending is not None:
                ...
                if int(s.group("index"))==idx:
                    ...
                    # Portmon field is treated as per-operation duration, never wall-clock.
                    clock+=dur
                pending=None
...
def main():
    ...
    out={"schema":"omegas.atlas.portmon-index.v1","clock_semantics":"cumulative completed-operation duration; no Portmon field is treated as absolute timestamp","autocal":{k:v for k,v in a.items() if k not in ("request_counts","examples")},"baseline":{k:v for k,v in b.items() if k not in ("request_counts","examples")},"candidate_count":len(candidates),"candidates":candidates[:256],"protocol_leads":leads}
    args.out.mkdir(parents=True,exist_ok=True)
    (args.out/"portmon-index.json").write_text(json.dumps(out,indent=2,ensure_ascii=False)+"\n",encoding="utf-8")
    print(json.dumps({"autocal":out["autocal"],"baseline":out["baseline"],"candidates":len(candidates),"lead_observations":leads},indent=2))
