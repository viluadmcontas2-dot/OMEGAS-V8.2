...
RESOURCE_RE = re.compile(r"^\\s*resource\\s+(\\S+)\\s+(?:→\\s+)?([A-Za-z_]\\w*):([^\\s(]+)", re.I)
...
def parse(text: str):
    ...
    for line in text.splitlines():
        ...
        if section == "fields":
            ...
            if m:
                classes[current]["published_methods"].append(
                    {
                        "va": int(m.group(1), 16),
    ...
    for line in text.splitlines():
        ...
        if obj is not None:
            ...
            if pm:
                obj["properties"][pm.group(1)] = clean_value(pm.group(2))

    method_candidates = {}
    for class_name, class_data in classes.items():
        for method in class_data["published_methods"]:
            method_candidates.setdefault(method["name"], []).append(
                ...
                action_bindings.append(
                    {
                        ...
                        "object": obj["name"],
                        "action": val,
                        "resolved_object": val in object_names,
                    }
                )

        if isinstance(obj["properties"].get("SerialCode"), int):
            serial_objects.append(
                {
                    "resource": obj["resource"],
                    "object": obj["name"],
                    "object_class": obj["class"],
                    "serial_code": obj["properties"]["SerialCode"],
                    "serial_hex": f"0x{obj['properties']['SerialCode']:04x}",
                    "file_section": obj["properties"].get("FileSection"),
                    "file_key": obj["properties"].get("FileKeyName"),
                }
            )

        if "Series" in obj["class"] or obj["class"] in {"TChart", "TShape"}:
    ...
    return {
        "schema": "omegas.atlas.semantic-index.v2",
        "classes": classes,
        "fields": fields,
        "resources": resources,
        "objects": objects,
        ...
        "counts": {
            "classes": len(classes),
            "fields": len(fields),
            "published_methods": sum(len(v) for v in method_candidates.values()),
            "resources": len(resources),
            "objects": len(objects),
            "event_bindings": len(event_bindings),
            "action_bindings": len(action_bindings),
            "serial_objects": len(serial_objects),
            "visual_objects": len(visual_objects),
            "object_field_links": len(object_field_links),
            "unresolved_event_bindings": len(unresolved_events),
            "unresolved_action_bindings": len(unresolved_actions),
        },
    }
...
def main():
    ...
    payload = parse(args.input.read_text(encoding="utf-8", errors="replace"))
    required = {"TAutoCalDM", "TAutoCalUI"}
    missing = sorted(required - set(payload["classes"]))
    if missing:
        raise SystemExit("missing required AutoCal classes: " + ", ".join(missing))
    if payload["counts"]["fields"] < 100:
        raise SystemExit("implausibly small AutoCal field corpus")
    if payload["counts"]["published_methods"] < 40:
        raise SystemExit("implausibly small AutoCal method corpus")
    if payload["counts"]["serial_objects"] < 10:
        raise SystemExit("implausibly small AutoCal serial corpus")
