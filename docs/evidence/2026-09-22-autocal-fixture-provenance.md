...
| `tests/fixtures/progbase-autocal-action-map-v1.json` | ProgBase original + Lognovo + Portmon AutoCal | DFM action metadata + handler/common-sender disassembly; capture presence recorded separately | ORIGINAL_DERIVED semantic/wire identity evidence | allowed for action identity and mode; physical effects only where separately observed | AutoCal action/oracle contracts |
