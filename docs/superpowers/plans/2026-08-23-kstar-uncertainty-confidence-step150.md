...
## Goal
Extend the existing Physics K* seam without creating a second estimator. Preserve `KStarEstimator` as the only target estimator, reuse `PlantGain`, `PhysicsUncertaintyOracle`, `PhysicsOracleValidator`, and `ConditionalActuatorTargets`, and add a pure calibration layer for uncertainty, evidence stage, and decomposed confidence.
...
- `PhysicsUncertaintyOracle`: canonical deterministic Monte-Carlo uncertainty oracle and plant-gain posterior authority.
- `PhysicsOracleValidator`: existing bootstrap/GUM/holdout validation; remains single-purpose and is not duplicated.
...
## TDD contract
1. RED: require a pure `KStarObservationCalibration` layer and canonical-oracle propagation API that do not exist yet.
2. Physics invariants:
   ...
   - a `g≈1` prior alone remains UNKNOWN until an informative intervention promotes the posterior.
3. Uncertainty:
...
6. Oracle:
   - extend the existing `PhysicsUncertaintyOracle` with deterministic seeded Monte-Carlo log-space propagation matching the explicit Step150 uncertainty decomposition;
   - keep `PhysicsOracleValidator` unchanged from its prior blob;
   - compare runtime analytic 95% interval with the canonical oracle in tests; tolerance is test-only numerical validation, never a runtime actionability threshold.
7. Regression:
   - existing KStarEstimator evidence-role/self-comparison/no-weight/unknown-gain behavior remains unchanged;
   - existing `PlantGainPosterior` remains the posterior authority;
...
- property/fuzz over uncertainty monotonicity, 4→6 stage, sign, gain prior, unknown/NaN and confidence monotonicity;
- deterministic Monte-Carlo cross-check against the analytic interval;
