...
class V82IntegralRegressionContract(unittest.TestCase):
    ...
    def test_predictor_consumes_canonical_predictions_without_second_engine_or_feedback(self):
        surface = read('app/src/main/java/com/omegas/prohub/learning/PredictorSurface.kt')
        access = read('app/src/main/java/com/omegas/prohub/service/V7CalibrationAccess.kt')
        ...
        self.assertIn('mapResidualPredictions', surface)
        self.assertIn('predictionByCell', surface)
        self.assertIn('OMEGAS_CONTINUOUS_RESIDUAL_FIELD', surface)
        self.assertNotIn('ContinuousResidualField.predict(', surface)
        self.assertNotIn('AssistedCalibrationAdvisor.analyze(', surface)
        self.assertNotIn('KWriteManager', surface)
        self.assertIn('PredictorSurface.build(learning, map)', access)
        self.assertNotIn('PredictorInterpolator', access)
