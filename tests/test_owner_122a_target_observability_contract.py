...
PROBE = ROOT / "app/src/main/java/com/omegas/prohub/autocal/AutoCalProbeMetrics.kt"
...
def test_122a_target_receipt_is_observational_and_bounded():
    ...
    assert 'probeMaxTelemetryGapMs' in target
...
def test_122a_probe_gap_flows_into_same_target_receipt():
    probe = PROBE.read_text(encoding="utf-8")
    assert 'AutoCal122ATargetMetrics.updateProbeMetrics(snapshot())' in probe
    assert 'lastTelemetryGapMs = gap' in probe
    assert 'maxTelemetryGapMs = maxOf' in probe
