...
DUAL_MATURITY = ROOT / 'app/src/main/java/com/omegas/prohub/autocal/NativeAutoCalDualFuelMaturityObserver.kt'
...
class NativeAutoCalContract(unittest.TestCase):
    def setUp(self):
        ...
        self.dual_maturity = DUAL_MATURITY.read_text('utf-8')
    ...
    def test_native_maturity_is_read_only_banded_monotonic_and_deduplicated(self):
        self.assertIn('AutoCalProtocol.NUM_BUF_UPD_GAS', self.dual_maturity)
        self.assertIn('reason = \"AutoCal maturidade GNV\"', self.dual_maturity)
        self.assertIn('workClass = Mp48WorkClass.READ_ONLY', self.dual_maturity)
        self.assertIn('elapsedRealtime()', self.dual_maturity)
    ...
    def test_anchor_correlation_uses_real_fuel_same_session_and_never_invents_position(self):
        self.assertIn('it.fuel.uppercase() in expectedFuel', self.correlator)
        self.assertIn('it.plausible', self.correlator)
        self.assertIn('it.sessionId == sessionId', self.correlator)
        ...
        self.assertIn('.put(\"correlatedGasMs\", correlation.gasMsDiagnostic', self.projector)
        self.assertIn('.put(\"correlatedFuel\", correlation.sourceFuel.name)', self.projector)
        self.assertIn('.put(\"correlatedFrameElapsedMs\", correlation.correlatedFrameElapsedMs', self.projector)
    ...
    def test_native_learning_anchor_requires_reliable_correlation_and_has_no_writer(self):
        ...
        self.assertIn('fuel.uppercase() in setOf(\"GNV\", \"CNG\", \"PETROL\", \"GASOLINA\")', self.anchor)
        ...
        self.assertIn('anchors.containsKey(anchor.fingerprint) || anchor.overlapKey in overlaps', self.anchor)
