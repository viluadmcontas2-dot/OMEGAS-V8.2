...
const mutants = [
  ...
  ['levels-autocal-leak', 'app/src/main/java/com/omegas/prohub/autocal/AutoCalJavascriptBridge.kt',
    'val manualSnapshot = manual?.latestSnapshotJson() ?: JSONObject().put("available", false)',
    'val manualSnapshot = manual?.latestSnapshotJson() ?: JSONObject().put("available", false)\n        val forbiddenLevelsLeak = JSONObject(service.telemetryStore.liveJson())'],
