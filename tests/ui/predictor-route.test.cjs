...
assert.match(predictor, /this\.store = app\.store/);
assert.match(predictor, /this\.router = app\.router/);
...
assert.match(map, /writePrepared\(\)/);
assert.match(map, /this\.editor\.buildReview\(\)/);
assert.equal(map.includes('openReview()'), false);
assert.equal(map.includes('writeReview()'), false);
