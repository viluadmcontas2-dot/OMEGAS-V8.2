...
- Antes de qualquer verdict que possa liberar dependente material, executor **e audit run processualmente independente** devem bootar pelo Governance EntryPoint, resolver o `CONTRACT REGISTRY`, carregar `GLOBAL-LEDGER-001`, `docs/contracts/transversal-pass-fail-gate.json` e `docs/contracts/TRANSVERSAL_PASS_FAIL_GATE.md`.
- `AUDIT_INDEPENDENCE=PROVENANCE_BASED`: independência pertence a `AUDIT_EPOCH_ID + AUDIT_RUN_ID + AUDIT_SCOPE_FINGERPRINT`, nunca a pessoa/modelo/chat/sessão. O audit run deve usar `AUDITOR_MODE=READ_ONLY_NORMATIVE` e `AUDITOR_NORMATIVE_WRITES=0`; o mesmo agente/modelo pode auditar quando inicia run novo e satisfaz integralmente essas condições.
...
- Falta de leitura, child/subpage/slice material não enumerado, aplicabilidade não classificada, requirement aplicável sem evidência, helper sem consumidor real, teste apenas string/grep para comportamento executável, benchmark host apresentado como RK3326, Prediction usada como Observation, harness quebrado usado como prova, proveniência de auditoria ausente/reutilizada, `NORMATIVE_AUDITED_MUTATION` no audit run ou `META_AUDIT_RUN_ID == AUDIT_RUN_ID` tornam o gate automaticamente não-PASS.
- Estados adicionais obrigatórios: `STALE_BY_GOVERNANCE` e `STALE_BY_EVIDENCE`. Qualquer PASS histórico sem receipt transversal + audit run independente por proveniência + meta-audit distinto vira `STALE_BY_GOVERNANCE` até reauditoria no SHA remoto exato.
- Implementação/migração fecha no máximo em `IMPLEMENTED_AWAITING_AUDIT`; verdict final requer audit run independente por proveniência e `META_AUDIT=PASS` em `META_AUDIT_RUN_ID` distinto. Só essa cadeia fresca libera owner/extensão/gate dependente. Nenhuma urgência, sequência numérica ou CI verde substitui essa regra.
