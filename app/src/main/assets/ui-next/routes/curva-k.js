...
function renderCurve(ctx, state) {
  ...
  const curve = state.curveK || {};
  if (!root) return;
  if (curve.state !== 'READY') {
    root.className = 'empty-state';
    root.innerHTML = `<div><strong>${curve.state === 'BUSY' ? 'Lendo Curva K' : 'Curva não confirmada'}</strong>São necessários exatamente 30 pontos reais antes da edição.</div>`;
    return;
  }
  ...
  root.innerHTML = `<section class="curve-points" aria-label="30 pontos da Curva K">
    ${(curve.points || []).map((point) => {
      ...
      return `<button class="curve-point${prep ? ' prepared' : ''}" type="button" data-index="${point.index}"><small>${format(point.petrolMs,2)} ms</small><b>${prep ? format(prep.after,3) : format(point.factor,3)}</b><span>${prep ? 'preparado' : 'ECU'}</span></button>`;
    }).join('')}
    </section>
    <section class="learning-now" id="curve-detail"><span class="section-kicker">Curva K global</span><strong class="learning-state">Toque num ponto</strong><p class="learning-reason">O eixo é Petrol Inj.; nenhum ponto desta curva é uma célula do Mapa K.</p><div class="learning-meta"><span class="state-chip">30 pontos reais</span><span class="state-chip">${prepared.length} preparados</span></div></section>`;
...
function renderPointDetail(ctx, curve, index) {
  ...
  root.innerHTML = `<span class="section-kicker">Petrol Inj. ${format(point.petrolMs,2)} ms</span><strong class="learning-state">Fator K ${format(point.factor,3)}</strong><p class="learning-reason">Prepare uma proposta global. Nada é gravado ao tocar ou ajustar.</p>
    <div class="nudge-group"><button class="secondary-action" data-delta="-0.02">−0,02</button><button class="secondary-action" data-delta="-0.005">−0,005</button><button class="secondary-action" data-delta="0.005">+0,005</button><button class="secondary-action" data-delta="0.02">+0,02</button></div>
    <button class="primary-action" id="curve-review" type="button" ${curve.prepared?.length ? '' : 'disabled'}>Revisar pontos preparados</button>`;
