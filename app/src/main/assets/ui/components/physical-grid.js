(() => {
  (function(root) {
    ...
    class PhysicalGrid {
      ...
      build() {
        ...
        for (let row = 0; row < this.rows; row += 1) {
          ...
          for (let column = 0; column < this.columns; column += 1) {
            ...
            button.innerHTML = '<span class="cell-value">\xB7</span><small class="cell-subvalue"></small>';
      ...
      cell(row, column) {
        return this.cells.get(this.key(row, column)) || null;
      }
      resetVisual() {
        this.cells.forEach((cell) => {
          cell.className = "physical-cell";
          cell.style.removeProperty("--heat");
          if (cell._valueNode) cell._valueNode.textContent = "\xB7";
      ...
      updateCell(row, column, visual) {
        ...
        if (!cell) return;
        const value = visual || {};
        const text = value.text === void 0 ? void 0 : String(value.text);
        const subtext = value.subtext === void 0 ? void 0 : String(value.subtext || "");
        const heat = value.heat === void 0 ? void 0 : Math.max(0, Math.min(1, Number(value.heat) || 0));
        const tone = value.tone || "";
        const state = value.state || "";
        const hasData = value.hasData !== false && value.text !== void 0 && value.text !== "\xB7";
        const selected = value.selected === true;
        const preview = value.preview === true;
        const signature = [text, subtext, heat === void 0 ? "-" : heat.toFixed(3), tone, state, hasData ? 1 : 0, selected ? 1 : 0, preview ? 1 : 0].join("|");
        if (signature === cell._visualSignature) return;
        cell._visualSignature = signature;
        if (text !== void 0 && cell._valueNode && cell._valueNode.textContent !== text) cell._valueNode.textContent = text;
        if (value.subtext !== void 0 && cell._subvalueNode && cell._subvalueNode.textContent !== subtext) cell._subvalueNode.textContent = subtext;
        if (heat !== void 0) cell.style.setProperty("--heat", String(heat));
