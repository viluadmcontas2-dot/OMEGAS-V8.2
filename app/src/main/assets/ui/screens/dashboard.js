  ...
  (function(root) {
    ...
    class DashboardScreen {
      ...
      installLayout() {
        ...
        const grid = this.root.querySelector(".now-metric-grid");
        const levels = document.getElementById("dashLevelsRaw");
        if (grid && levels && !document.getElementById("dashVoltage")) {
          const voltageCard = document.createElement("article");
          voltageCard.className = "now-metric-card now-voltage-card";
          voltageCard.innerHTML = '<small>TENS\xC3O ECU</small><b id="dashVoltage">\u2014</b><span id="dashVoltageState">OBD 0142</span>';
          grid.insertBefore(voltageCard, levels.closest("article"));
        }
      }
      render(state) {
        ...
        const voltage = obdConnected ? obdValue(obd, ["moduleVoltageV", "controlModuleVoltage", "voltage"]) : null;
        ...
        text("dashFuel", fuel);
        text("dashVoltage", voltage === null ? "\u2014" : "".concat(fmt(voltage, 2), " V"));
        text("dashVoltageState", obdConnected ? voltage === null ? "aguardando 0142" : "OBD 0142" : "OBD offline");
