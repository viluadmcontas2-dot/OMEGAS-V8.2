  ...
  (function(root) {
    ...
    class DashboardScreen {
      ...
      render(state) {
        ...
        const fuel = telemetryValid ? fuelLabel(data.fuel || data.state || status.fuelState) : "\u2014";\n        const gnvLevelRaw = telemetryValid ? finite(data.level_raw != null ? data.level_raw : data.levelRaw) : null;
        ...
        text("dashCell", row !== null && column !== null ? "".concat(row + 1, "\xD7").concat(column + 1) : "\u2014");\n        text("dashGnvLevel", gnvLevelRaw === null ? "\u2014" : Math.round(gnvLevelRaw).toLocaleString("pt-BR"));
