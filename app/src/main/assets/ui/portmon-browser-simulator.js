...
(() => {
  ...
  (function installPortmonBrowserSimulator(root) {
    ...
    const rows = Array.from({ length: 12 }, (_, row) => Array.from({ length: 12 }, (_2, column) => 110 + row + column));
    const extraRow = Array(12).fill(77);
    const axes = {
      petrolBins: [2, 2.5, 3, 3.5, 4.5, 6, 8, 10, 12, 14, 16, 18],
      rpmBins: [850, 1350, 1850, 2500, 3e3, 3500, 4e3, 4500, 5e3, 5500, 6e3, 6500]
    };
    const rawFrames = [];
    let connected = true;
    let mode = "NORMAL";
    ...
    const hex = (bytes) => bytes.map((value) => (value & 255).toString(16).padStart(2, "0").toUpperCase()).join(" ");
    const checksum = (bytes) => bytes.reduce((sum, value) => sum + value & 255, 0);
    const withChecksum = (bytes) => [...bytes, checksum(bytes)];
    const readRowFrame = (row) => withChecksum([42, 84, 0, row]);
    const writeCellFrame = (row, column, value) => withChecksum([20, 84, 0, row, column, value]);
    const ackFrame = (request) => [...request, 83, 1, 0, checksum(request)];
    const hash = () => rows.flat().join("-");
    const json = (value) => JSON.stringify(value);
    const record = (kind, request, response, details = {}) => rawFrames.push(__spreadValues({
      index: rawFrames.length + 1,
      kind,
      request: hex(request),
      response: response ? hex(response) : null
    }, details));
    function readMap() {
      ...
      for (let row = 0; row < 13; row += 1) {
        ...
        const response = withChecksum([...request, 83, ...values]);
    ...
    function writeBatch(payload) {
      ...
      for (const item of payload) {
        if (!Number.isInteger(item.row) || item.row < 0 || item.row >= 12 || !Number.isInteger(item.column) || item.column < 0 || item.column >= 12) {
          throw new Error("C\xE9lula fora da \xE1rea edit\xE1vel");
        }
      ...
      payload.forEach((item) => {
        rows[item.row][item.column] = item.target;
      });
    ...
    root.OmegasNative = {
      ...
      getStatus: () => json({
        ...
        usbDevice: "ECU MP48 simulada",
        usbPermissionPending: false,
        ecuState: connected ? "SIMULATED" : "OFFLINE",
        fuelState: "SIMULADO",
        rpm: 0,
        petrolMs: 0,
        gasMs: 0,
        mapBar: 0,
        directTelemetryAgeMs: connected ? 0 : -1,
        simulated: true,
        lastError: writeStatus.ok === false ? writeStatus.message : ""
      }),
      getFullEngineSnapshot() {
        ...
        return json({
          ok: connected,
          simulated: true,
          sequence: (frame == null ? void 0 : frame.sequence) || 0,
          updatedAt: Date.now(),
          ageMs: 0,
          live: { fuel: "SIMULADO", rpm: 0, petrol_ms: 0, gas_ms_diagnostic: 0, load_bar: 0 },
          portmon: frame,
          rawFrames: rawFrames.slice(-20),
      ...
      startKBatchWrite(payloadJson, maxStep, pauseMs, reason) {
        ...
        try {
          ...
          return json({ ok: false, started: false, error: "Lote inv\xE1lido" });
        }
        if (!Array.isArray(payload) || payload.length < 1 || payload.length > 16) return json({ ok: false, started: false, error: "Lote deve conter de 1 a 16 c\xE9lulas" });
      ...
      setPortmonSimulatorMode(nextMode) {
        ...
        if (!allowed.includes(nextMode)) return json({ ok: false, error: "Modo inv\xE1lido" });
        mode = nextMode;
      ...
      exportLearningArchive() {
      },
      importLearningArchive() {
      },
      exportLogs() {
      }
    };
