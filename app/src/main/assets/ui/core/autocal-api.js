(() => {
  (function(root) {
    ...
    function invoke(name, args, fallback) {
      ...
      try {
        return parse(fn.apply(bridge, args || []), fallback);
      } catch (error) {
        return { ok: false, error: (error == null ? void 0 : error.message) || String(error), automatic: false, manualOnly: true };
      }
    }
    ns.AutoCalApi = {
      available: () => !!root.OmegasAutoCal,
      identity: () => invoke("getIdentity", [], {}),
      status: () => invoke("getNativeMonitorStatus", [], { ok: false, error: "AutoCal nativo indispon\xEDvel" }),
