 ...
 * arquivos já produzidos para Documents/Omegas/AutoCal e nunca cria polling,
 * captura paralela, pruning ou autoridade científica.
 */
class DocumentsSessionMirror(private val context: Context) {
    companion object {
        const val PUBLIC_ROOT = "Documents/Omegas/AutoCal"
    }
        ...
        if (!isAvailable()) {
            return fail(
                ...
                if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.P) {
                    "Permissão de armazenamento necessária para Documentos/Omegas"
                } else {
    ...
    private fun syncScoped(source: File, safeSession: String) {
        ...
        val relativePath = "$PUBLIC_ROOT/$safeSession/"
        ...
        if (targetUri == null) {
            ...
            targetUri = resolver.insert(collection, values)
                ?: error("Não foi possível criar " + source.name + " em Documentos/Omegas")
            targetSize = 0L
        }
