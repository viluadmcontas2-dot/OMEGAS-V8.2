...
/**
 * Camada de admissão sobre a authority serial MP48 existente.
 *
 * Não cria executor, thread, fila, transporte ou polling próprio. Leituras
 * secundárias recebem admissão não bloqueante; MANUAL_WRITE/SAFETY possuem
 * capacidade reservada e espera limitada pelo timeout do chamador.
 */
class Mp48BackpressureScheduler(
    ...
    override fun recentTelemetryFrames(
        fromElapsedMs: Long,
        toElapsedMs: Long,
    ) = delegate.recentTelemetryFrames(fromElapsedMs, toElapsedMs)
