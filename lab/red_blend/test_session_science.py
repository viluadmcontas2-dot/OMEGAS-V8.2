...
from lab.red_blend.session_science import attribute_session_mixture, decompose_sessions, leave_one_session_out

def make_sessions(means, sigma=0.03, n=80, seed=1):
    rng = random.Random(seed)
    return {f"s{i}": [rng.gauss(mean, sigma) for _ in range(n)] for i, mean in enumerate(means)}
...
class SessionScienceTest(unittest.TestCase):
    ...
    def test_shifted_tight_sessions_expose_between_session_drift_and_loso_penalty(self):
        ...
        self.assertGreater(shifted_loso.median_abs_relative_error, stable_loso.median_abs_relative_error * 4.0)
    ...
    def test_true_within_session_bimodality_survives_session_centering(self):
        ...
        groups = {}
        for i in range(6):
            groups[f"s{i}"] = [rng.gauss(3.80, 0.02) for _ in range(60)] + [rng.gauss(4.20, 0.02) for _ in range(60)]
        attribution = attribute_session_mixture(groups)
        self.assertTrue(attribution.pooled.is_multimodal)
