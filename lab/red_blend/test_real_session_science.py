...
class RealSessionScienceTest(unittest.TestCase):
    ...
    def test_sparse_two_session_gasoline_multimodal_candidate_cannot_claim_transfer(self):
    ...
    def test_dense_gasoline_regions_expose_session_decomposition_and_loso(self):
        ...
        for key in ((5, 9), (5, 10)):
            ...
            self.assertIn(
                region.mixture_attribution.interpretation,
                {
                    "NO_STRONG_POOLED_MULTIMODALITY",
                    "SESSION_OFFSETS_DOMINANT_CANDIDATE",
                    "WITHIN_SESSION_REGIME_CANDIDATE",
                    "MIXTURE_ATTRIBUTION_AMBIGUOUS",
                },
            )

    def test_real_session_audit_is_deterministic(self):
        a = audit_real_session_regions(
            self.episodes,
            fuel="GASOLINA",
            min_samples=4,
            min_independent_sessions=3,
        )
        b = audit_real_session_regions(
            self.episodes,
            fuel="GASOLINA",
            min_samples=4,
            min_independent_sessions=3,
